...
//
//   web  : all-time agent-vs-human page views to coldview.wiki, counted
//          server-side by _middleware.js into D1 (cumulative since launch).
//   pypi : install counts for `coldview-ai` (pypistats public API), with a
...
async function getPypi(env) {
  ...
  try {
    const resp = await fetch(
      "https://pypistats.org/api/packages/coldview-ai/recent",
      { headers: { "User-Agent": "coldview-wiki" } },
    );
    if (resp.ok) {
