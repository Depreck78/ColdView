...
Coldview. It does not replace `CONTRIBUTING.md`, `SECURITY.md`, or the pull
...
- MCP entry point: `coldview-mcp` / `agent/mcp_server.py`.
- CLI entry point: `coldview` / `agent/cli/`.
...
- write real credentials into `agent/.env`, `~/.coldview/`, token caches, or
  external services,
