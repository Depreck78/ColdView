...
describe("apiAuth", () => {
  ...
  describe("getApiAuthKey", () => {
    ...
    it("returns stored key", () => {
      localStorage.setItem("coldview_api_auth_key", "my-secret");
  ...
  describe("setApiAuthKey", () => {
    it("stores trimmed value", () => {
      ...
      expect(localStorage.getItem("coldview_api_auth_key")).toBe("abc-123");
    });
    it("removes key when value is empty/whitespace", () => {
      setApiAuthKey("abc");
      setApiAuthKey("   ");
      expect(localStorage.getItem("coldview_api_auth_key")).toBeNull();
    });
