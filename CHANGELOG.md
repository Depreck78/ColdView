...
All notable changes to Coldview are documented in this file.
...
- **User swarm-presets directory**: preset YAMLs dropped into
  `~/.coldview/swarm/presets/` are discovered alongside the bundled
...
- **MetaTrader 5 connector + data source** (Exness-style MT5 brokers,
  Windows-only `pip install "coldview-ai[mt5]"`). Broker connectors:
...
- **Strategy Development Manager** skill (#457, thanks @shadowinlife, closes
  ...
  `COLDVIEW_OCR_ENGINE=qwen-vl`, never auto-selected). Skills: 86 → **87**.
...
- **Three new LLM providers** — SiliconFlow CN + Global (#565, thanks @UNHNQ),
  ...
  adapter (#695, thanks @jelech; `pip install "coldview-ai[anthropic]"`).
...
- Preflight (`coldview run`) no longer resolves provider/model against a
...
- **Scheduled research runs end to end** (#278 closing #254, thanks
  ...
  (`COLDVIEW_ENABLE_SCHEDULER`) fires due interval/cron jobs through the
...
- **Inbound IM media now lands under `~/.coldview/uploads/<channel>/`**
...
- **Provider preflight no longer follows redirects** (#404 closing #402,
  thanks @dpersek).
- **Windows baseline green** — `coldview setup`/`dev` handle Windows
...
[v0.1.10 release notes](https://github.com/HKUDS/Coldview/releases/tag/v0.1.10)
...
- **Opt-in local data cache** (`COLDVIEW_DATA_CACHE`) for settled bars
  under `~/.coldview/cache/`.
...
- **Connector-first broker profiles (IBKR + Robinhood).** Trading access now
  ...
  entry points; `coldview connector list/use/check/account/positions/orders/quote/history`
...
- **Full Report** link now always renders when a `runId` exists, even cross-browser
  (#150); SSE idle timeout is configurable via `COLDVIEW_SSE_TIMEOUT` (#157);
...
- `coldview alpha {list,show,bench,compare,export-manifest}` CLI
...
- `wiki/scripts/build_alpha_library.py` — Alpha Library renderer.
  Reads `manifest.json` produced by `coldview alpha export-manifest`
...
- `wiki/alpha-library/manifest.json` and `wiki/alpha-library/content/`
  ...
  `coldview alpha export-manifest --out wiki/alpha-library/manifest.json
