...
LABEL org.opencontainers.image.title="Coldview" \
    ...
    org.opencontainers.image.source="https://github.com/HKUDS/Coldview" \
...
# Runtime should not run as root. `coldview` owns the writable app-data dirs so
# named volumes inherit usable permissions. `coldview-sandbox` is an unprivileged
# system account (no home, no shell) that runner.py drops into via
# subprocess.run(user="coldview-sandbox") to execute LLM-generated code with the
# least privilege — created here by fixed contract, not otherwise used.
RUN useradd --create-home --shell /usr/sbin/nologin coldview \
    && useradd --system --no-create-home --shell /usr/sbin/nologin --uid 10001 coldview-sandbox \
    && mkdir -p agent/runs agent/sessions agent/uploads agent/.swarm/runs /home/coldview/.coldview \
    && chown -R coldview:coldview /app /home/coldview/.coldview
USER coldview
...
CMD ["coldview", "serve", "--host", "0.0.0.0", "--port", "8899"]
