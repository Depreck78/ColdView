...
_SWARM_AGENT_CONFIG_ENV_VAR = "COLDVIEW_SWARM_AGENT_CONFIG"
...
def _resolve_swarm_agent_config_path(
    *,
    ...
    1. ``COLDVIEW_SWARM_AGENT_CONFIG`` env var — absolute override hatch
    ...
    Args:
        runtime_root: Override the directory the on-disk lookup uses. Defaults
            to ``~/.coldview``. Tests pass a ``tmp_path`` here to keep
            assertions hermetic.
