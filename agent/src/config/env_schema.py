"""Single source of truth for all Coldview environment variable defaults.

This module defines Pydantic models for every environment variable consumed by
the Coldview agent, grouped by functional category.  Each field carries the
...
class LLMConfig(_EnvBase):
    ...
    coldview_deepseek_adapter: str = Field(alias="COLDVIEW_DEEPSEEK_ADAPTER", default="auto")
...
class DataConfig(_EnvBase):
    ...
    coldview_iwencai_key: str = Field(alias="COLDVIEW_IWENCAI_KEY", default="")
    coldview_sec_ua: str = Field(alias="COLDVIEW_SEC_UA", default="")
    coldview_data_cache: EnvBool = Field(alias="COLDVIEW_DATA_CACHE", default=False)
    coldview_data_cache_root: str = Field(alias="COLDVIEW_DATA_CACHE_ROOT", default="")
...
class OcrConfig(_EnvBase):
    ...
    coldview_ocr_engine: str = Field(alias="COLDVIEW_OCR_ENGINE", default="auto")
    coldview_ocr_llm_model: str = Field(alias="COLDVIEW_OCR_LLM_MODEL", default="")

    @model_validator(mode="before")
    @classmethod
    def _alias_legacy_env_vars(cls, data: dict) -> dict:
        """Backward compat: COLDVIEW_OCR_QWEN_MODEL → COLDVIEW_OCR_LLM_MODEL.
        ...
        old_val = os.getenv("COLDVIEW_OCR_QWEN_MODEL", "")
        new_val = (
            data.get("coldview_ocr_llm_model")
            or os.getenv("COLDVIEW_OCR_LLM_MODEL", "")
        )
        if old_val and not new_val:
            logging.getLogger(__name__).warning(
                "COLDVIEW_OCR_QWEN_MODEL is deprecated; "
                "use COLDVIEW_OCR_LLM_MODEL instead. "
                "Alias will be removed in a future release."
            )
            data["coldview_ocr_llm_model"] = old_val
        return data
...
class APIConfig(_EnvBase):
    ...
    coldview_api_key: str = Field(alias="COLDVIEW_API_KEY", default="")
    ...
    coldview_mcp_allowed_hosts: str = Field(
        alias="COLDVIEW_MCP_ALLOWED_HOSTS", default="",
    )
    enable_session_runtime: EnvBool = Field(alias="ENABLE_SESSION_RUNTIME", default=True)
    coldview_trust_docker_loopback: EnvBool = Field(
        alias="COLDVIEW_TRUST_DOCKER_LOOPBACK", default=False,
    )
    coldview_enable_shell_tools: EnvBool = Field(
        alias="COLDVIEW_ENABLE_SHELL_TOOLS", default=False,
    )
    coldview_allowed_file_roots: str = Field(
        alias="COLDVIEW_ALLOWED_FILE_ROOTS", default="",
    )
    coldview_allowed_write_roots: str = Field(
        alias="COLDVIEW_ALLOWED_WRITE_ROOTS", default="",
    )
    coldview_allowed_run_roots: str = Field(
        alias="COLDVIEW_ALLOWED_RUN_ROOTS", default="",
    )
    coldview_api_url: str = Field(
        alias="COLDVIEW_API_URL", default="http://127.0.0.1:8000",
    )
...
class AgentTuningConfig(_EnvBase):
    ...
    coldview_tool_timeout_seconds: float = Field(
        alias="COLDVIEW_TOOL_TIMEOUT_SECONDS", default=1800.0,
    )
    coldview_goal_max_continuations: int = Field(
        alias="COLDVIEW_GOAL_MAX_CONTINUATIONS", default=3,
    )
    coldview_sse_timeout: int = Field(alias="COLDVIEW_SSE_TIMEOUT", default=90)
    content_filter_warning_threshold: float = Field(
        alias="CONTENT_FILTER_WARNING_THRESHOLD", default=0.05,
    )
    coldview_enable_advisory: EnvBool = Field(
        alias="COLDVIEW_ENABLE_ADVISORY", default=False,
    )
    coldview_enable_scheduler: EnvBool = Field(
        alias="COLDVIEW_ENABLE_SCHEDULER", default=False,
    )
    coldview_channels_auto_start: EnvBool = Field(
        alias="COLDVIEW_CHANNELS_AUTO_START", default=False,
    )
    coldview_disable_bottleneck: EnvBool = Field(
        alias="COLDVIEW_DISABLE_BOTTLENECK", default=False,
    )
    coldview_bench_workers: int = Field(alias="COLDVIEW_BENCH_WORKERS", default=0)
    coldview_search_backends: str = Field(alias="COLDVIEW_SEARCH_BACKENDS", default="")
    coldview_search_bing_fallback: EnvBool = Field(
        alias="COLDVIEW_SEARCH_BING_FALLBACK", default=True,
    )
...
class PathConfig(_EnvBase):
    ...
    coldview_hypotheses_path: str = Field(alias="COLDVIEW_HYPOTHESES_PATH", default="")
    coldview_goal_db_path: str = Field(alias="COLDVIEW_GOAL_DB_PATH", default="")
    coldview_swarm_agent_config: str = Field(
        alias="COLDVIEW_SWARM_AGENT_CONFIG", default="",
    )
    allow_session_mcp_servers: EnvBool = Field(alias="ALLOW_SESSION_MCP_SERVERS", default=False)
    coldview_theme: str = Field(alias="COLDVIEW_THEME", default="")
    vibe_goal_session_id: str = Field(alias="VIBE_GOAL_SESSION_ID", default="")
    coldview_strategy_store_db_path: str = Field(
        alias="COLDVIEW_STRATEGY_STORE_DB_PATH", default="",
    )
...
class EnvConfig(_EnvBase):
    ...
    def _resolve_api_key_alias(self) -> "EnvConfig":
        """Copy ``COLDVIEW_API_KEY`` to ``api_auth_key`` when only the alias is set.

        The CLI historically reads ``COLDVIEW_API_KEY`` first and falls back to
        ...
        """
        if self.api.coldview_api_key and not self.api.api_auth_key:
            self.api.api_auth_key = self.api.coldview_api_key
        return self
