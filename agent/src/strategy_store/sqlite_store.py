...
_DEFAULT_DB_PATH = Path.home() / ".coldview" / "strategy_store.db"
...
def _default_db_path() -> Path:
    ...
    raw_path = get_env_config().paths.coldview_strategy_store_db_path.strip()
    if raw_path:
    ...
    def __init__(self, db_path: Path | str | None = None) -> None:
        ...
        Args:
            db_path: SQLite database path.  When omitted,
                ``COLDVIEW_STRATEGY_STORE_DB_PATH`` can override the
                default ``~/.coldview/strategy_store.db``.
        """
        self.db_path = (
