...
from ``COLDVIEW_IWENCAI_KEY``. The tool is silently excluded from the registry
...
class IWenCaiSearchTool(BaseTool):
    ...
    def check_available(cls) -> bool:
        ...
        Returns:
            ``True`` when ``COLDVIEW_IWENCAI_KEY`` is set to a non-empty
            value; ``False`` otherwise, which silently excludes the tool from
            the registry. Never raises.
        """
        return bool(get_env_config().data.coldview_iwencai_key)
    ...
    description = (
        ...
        "question. Read-only; requires the COLDVIEW_IWENCAI_KEY access key. "
    ...
    def execute(self, **kwargs: Any) -> str:
        ...
        """
        key = get_env_config().data.coldview_iwencai_key or None
        if not key:
            return self._error(
