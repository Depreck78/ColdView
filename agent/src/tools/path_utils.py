...
_ALLOWED_FILE_ROOTS_ENV = "COLDVIEW_ALLOWED_FILE_ROOTS"
_ALLOWED_RUN_ROOTS_ENV = "COLDVIEW_ALLOWED_RUN_ROOTS"
...
def safe_path(p: str, workdir: Path) -> Path:
    ...
    # Expand ~ so home-relative paths (e.g. ~/.coldview/scripts/foo.py)
...
def _configured_file_roots() -> list[Path]:
    ...
    raw = get_env_config().api.coldview_allowed_file_roots
    roots: list[Path] = []
...
def _default_file_roots() -> list[Path]:
    ...
    return [
        ...
        cwd / "uploads",
        cwd / "data",
        home / ".coldview" / "uploads",
        home / ".coldview" / "imports",
    ]
...
_ALLOWED_WRITE_ROOTS_ENV = "COLDVIEW_ALLOWED_WRITE_ROOTS"


def allowed_write_roots() -> list[Path]:
    """Return all roots allowed for file writes and edits."""
    raw = get_env_config().api.coldview_allowed_write_roots
...
def _allowed_run_roots() -> list[Path]:
    ...
    raw = get_env_config().api.coldview_allowed_run_roots
    configured: list[Path] = []
    for item in raw.split(","):
        item = item.strip()
        if not item:
