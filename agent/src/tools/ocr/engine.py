...
(any OpenAI-compatible provider) via environment variable COLDVIEW_OCR_ENGINE.

Third-party OCR engines can be installed as pip packages and are automatically
discovered via entry_points (group: coldview.ocr_engines).
"""
...
_ENTRY_POINT_GROUP = "coldview.ocr_engines"
...
def _get_ocr_choice() -> str:
    ...
    return get_env_config().ocr.coldview_ocr_engine.strip().lower()
...
def get_ocr_engine() -> OcrEngine | None:
    ...
    Engine selection via COLDVIEW_OCR_ENGINE:
...
def get_ocr_install_hint(engine: OcrEngine | None) -> str:
    ...
    lines.append("  # Cloud OCR: set COLDVIEW_OCR_ENGINE=llm-vision")
    lines.append("  # and configure a vision-capable LLM model")
    lines.append("")
    lines.append("Or set COLDVIEW_OCR_ENGINE=none to disable OCR.")
    return "\n".join(lines)
