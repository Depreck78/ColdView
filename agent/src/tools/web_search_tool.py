...
# Override (or pin to one engine) via COLDVIEW_SEARCH_BACKENDS.
...
class WebSearchTool(BaseTool):
    ...
    def execute(self, **kwargs: Any) -> str:
        ...
        backends = (get_env_config().agent_tuning.coldview_search_backends or _DEFAULT_BACKENDS).strip() or "auto"
        ...
        # COLDVIEW_SEARCH_BING_FALLBACK (default on).
        fb_err = "disabled"
        if get_env_config().agent_tuning.coldview_search_bing_fallback:
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    "Retry shortly, set COLDVIEW_SEARCH_BACKENDS to a different engine list, "
                    "set COLDVIEW_SEARCH_BING_FALLBACK=0 to disable CN fallback, or read a "
