...
# any of this.
_SANDBOX_USER = "coldview-sandbox"
# The only paths under ~/.coldview re-exposed into the ephemeral sandbox
...
_SANDBOX_RLIMIT_AS_MB_ENV = "COLDVIEW_SANDBOX_RLIMIT_AS_MB"
...
def _resolve_sandbox_credentials() -> tuple[str, str] | None:
    ...
    None (run without a UID drop) is returned whenever the ``coldview-sandbox``
...
def _prepare_sandbox_home(real_home: Path | None) -> Path:
    ...
    the ephemeral home re-exposes the narrow set of ``~/.coldview`` paths the
    ...
    """
    sandbox = Path(tempfile.mkdtemp(prefix="coldview-sandbox-home-"))
    if real_home is not None:
        src_root = real_home / ".coldview"
        if src_root.is_dir():
            dst_root = sandbox / ".coldview"
...
_RUNTIME_ENV_KEYS = frozenset(
    {
        ...
        "FMP_API_KEY",
        "FRED_API_KEY",
        "COLDVIEW_IWENCAI_KEY",
        "COLDVIEW_SEC_UA",
        "COLDVIEW_DATA_CACHE",
        "COLDVIEW_ALLOWED_RUN_ROOTS",
        "CCXT_EXCHANGE",
        "CCXT_TIMEOUT_MS",
        "CCXT_FETCH_BUDGET_S",
        "OKX_TIMEOUT_S",
        ...
        "FUTU_HOST",
        "FUTU_PORT",
        "COLDVIEW_EASTMONEY_MIN_INTERVAL",
        "COLDVIEW_SINA_MIN_INTERVAL",
        "COLDVIEW_STOOQ_MIN_INTERVAL",
        "COLDVIEW_YAHOO_MIN_INTERVAL",
        "COLDVIEW_SEC_MIN_INTERVAL",
        "COLDVIEW_FINNHUB_MIN_INTERVAL",
        "COLDVIEW_ALPHAVANTAGE_MIN_INTERVAL",
        "COLDVIEW_TIINGO_MIN_INTERVAL",
        "COLDVIEW_FMP_MIN_INTERVAL",
        "COLDVIEW_FRED_MIN_INTERVAL",
        "COLDVIEW_IWENCAI_MIN_INTERVAL",
        "COLDVIEW_THS_MIN_INTERVAL",
    }
    | _PROXY_ENV_KEYS
)
...
class Runner:
    ...
    def _run_sandboxed(self, cmd: list[str], run_kwargs: dict[str, Any]) -> "subprocess.CompletedProcess[str]":
        """Run the subprocess, dropping to ``coldview-sandbox`` when the host allows it.
    ...
    def execute(
        ...
        *,
        ...
        # HOME (so it can't read the persistent ~/.coldview secrets/state),
