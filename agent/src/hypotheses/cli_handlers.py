"""CLI handlers for ``coldview hypothesis {list,show,invalidate}``.
...
(and tests) can override via the ``COLDVIEW_HYPOTHESES_PATH`` env var or by
passing ``--path``.
"""
...
def add_subparser(subparsers: Any) -> argparse.ArgumentParser:
    ...
    hyp_parser.add_argument(
        ...
        help=(
            ...
            "COLDVIEW_HYPOTHESES_PATH env var)"
        ),
    )
...
def dispatch(args: argparse.Namespace) -> int:
    ...
    if sub is None:
        if _HYP_PARSER is not None:
            ...
            _err("hypothesis requires a subcommand. Try: coldview hypothesis list")
        return 1
