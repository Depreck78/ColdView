...
User-supplied presets are also discovered from ``~/.coldview/swarm/
...
USER_PRESETS_DIR = Path.home() / ".coldview" / "swarm" / "presets"
...
def load_preset(name: str) -> dict:
    ...
    Raises:
        ...
        FileNotFoundError: If the preset file does not exist in either the
            user directory (``~/.coldview/swarm/presets/``) or the
...
def list_presets() -> list[dict]:
    ...
    User presets (``~/.coldview/swarm/presets/``) are listed alongside the
