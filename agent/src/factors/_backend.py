...
:func:`__getattr__` so the ``COLDVIEW_DISABLE_BOTTLENECK`` env var
...
def _ensure_bottleneck() -> None:
    ...
    Reads ``COLDVIEW_DISABLE_BOTTLENECK`` through the lazy
    ...
    if get_env_config().agent_tuning.coldview_disable_bottleneck:
        return
    try:
