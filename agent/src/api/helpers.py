...
# helpers.py lives at agent/src/api/helpers.py — 4 levels up to Coldview/.
...
ENV_PATH = Path.home() / ".coldview" / ".env"
...
def _ensure_agent_env_file(path: Path | None = None) -> Path:
    ...
    if not env_path.exists():
        _atomic_write_secret(env_path, "# Created by Coldview Web UI settings.\n")
    return env_path
