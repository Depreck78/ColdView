...
# .env search order: ~/.coldview/.env → agent/.env → $CWD/.env
_ENV_CANDIDATES = [
    Path.home() / ".coldview" / ".env",
    AGENT_DIR / ".env",
    Path.cwd() / ".env",
]
...
# constants only.
_ENV_LABELS = ("~/.coldview/.env", "<AGENT_DIR>/.env", "<CWD>/.env")
...
def _deepseek_adapter_mode() -> str:
    ...
    mode = get_env_config().llm.coldview_deepseek_adapter.strip().lower()
    aliases = {
    ...
    *,
    ...
    try:
        ...
        raise RuntimeError(
            ...
            'extra: pip install "coldview-ai[anthropic]" (or pip install langchain-anthropic).'
        ) from exc
...
def _sync_provider_env() -> None:
    ...
    if provider in {"openai-codex", "openai_codex"}:
        ...
        # SDK-side env setup, not Coldview config reads
    ...
    if provider == "deepseek":
        ...
        if adapter_mode != "openai-compatible":
            ...
            if adapter_mode == "native":
                raise RuntimeError(
                    "COLDVIEW_DEEPSEEK_ADAPTER=native requires langchain-deepseek"
                )
