...
the Futu login, and the SDK speaks to it over a local socket. Coldview never
...
def build_config(profile_config: Mapping[str, Any] | None = None, overrides: Mapping[str, Any] | None = None) -> "FutuConfig":
    ...
    ``~/.coldview/futu.json``; the selected connector profile supplies the
...
def load_config() -> FutuConfig:
    """Load Futu settings from ``~/.coldview/futu.json``."""
    path = config_path()
    if not path.exists():
        return FutuConfig()
    try:
