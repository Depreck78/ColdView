...
class MT5Config:
    """MT5 connector connection settings (persisted in ``~/.coldview/mt5.json``).
...
def load_config() -> MT5Config:
    """Load MT5 settings from ``~/.coldview/mt5.json``."""
    path = config_path()
    if not path.exists():
        return MT5Config()
    try:
...
def _require_mt5() -> ModuleType:
    ...
    try:
        ...
        raise MT5DependencyError(
            ...
            '`pip install "coldview-ai[mt5]"` and run a local MT5 terminal.'
        ) from exc
    return MetaTrader5
