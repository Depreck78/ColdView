...
This reference is based on the frozen Coldview QVeris contract dated
...
`QVERIS_BASE_URL`; Coldview's default is the production API v1 endpoint from
the frozen contract.
...
| `session_id` | string | no | Optional Coldview session id. |
...
Executes one capability. Coldview must keep QVeris unavailable in `free`
...
| `session_id` | string | no | Correlates execution to Coldview session. |
...
Coldview clients should use a minimum request interval of 0.5 seconds and at
