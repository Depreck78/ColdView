...
description: Paid capability marketplace for global multi-asset data; use it when free Coldview sources lack coverage, depth, or provider quality, and keep free sources as the default for routine OHLCV.
---
# QVeris
...
Signup link: [QVeris via Coldview](https://qveris.ai/?ref=Vyjjo5G_1cAHJA).
...
- The default free route is Coldview's built-in public data stack; QVeris
...
Settings -> QVeris / `coldview data mode paid`. If QVeris is disabled, in
...
- Web: Settings -> QVeris.
- CLI: `coldview data status` or `coldview data usage`.
