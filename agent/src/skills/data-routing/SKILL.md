...
| mt5 | Forex & metals (your broker's MT5 feed) | Yes (running, logged-in MT5 terminal; optional `~/.coldview/mt5.json`) | Local terminal (Windows) | data-routing (runner-internal) |
...
| iWenCai NL search (问财) | `iwencai_search` | A-share | `COLDVIEW_IWENCAI_KEY` |
...
- **Key-gated sources need their env key** (`FINNHUB_API_KEY`,
  ...
  `COLDVIEW_IWENCAI_KEY`, `TUSHARE_TOKEN`). If the key is absent the tool/loader
