---
name: vnpy-export
description: Export a Coldview backtest strategy to a runnable vnpy CtaTemplate Python class — supports A-share equities, futures, and crypto via BarGenerator + ArrayManager.
category: tool
---

## Overview

This skill translates a Coldview strategy into a **vnpy `CtaTemplate` subclass** `.py` file
...
run a Coldview strategy inside vnpy's CTA backtester or live trading engine.
...
| Python (Coldview / pandas / ta-lib) | vnpy ArrayManager |
|----------------------------------------|-------------------|
...
| Coldview signal | Position check | vnpy call |
|---------------------|---------------|-----------|
...
When the Coldview strategy uses multiple timeframes (e.g., daily signal, hourly entry):
...
- [ ] Comment block at top of file notes the original Coldview `run_id` and instrument
