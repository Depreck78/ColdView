#!/usr/bin/env python3
"""Coldview API Server - RESTful API for finance research and backtesting.
...
app = FastAPI(
    title="Coldview API",
    description="Coldview API: natural-language finance research, backtesting, and swarm workflows",
    version=APP_VERSION,
    docs_url="/docs",
    redoc_url="/redoc"
)
...
async def _run_startup_preflight() -> None:
    ...
    if get_env_config().agent_tuning.coldview_channels_auto_start:
...
register_morning_routes(app)

# --- Ollama local runtime (status, preflight, one-click install) ---
from src.api.ollama_routes import register_ollama_routes  # noqa: E402
register_ollama_routes(app)

# --- Onboarding (first-run broker connect) ---
from src.api.onboarding_routes import register_onboarding_routes  # noqa: E402
register_onboarding_routes(app)

# --- AI provider helpers (list a connected provider's models) ---
...
# guarded separately by COLDVIEW_ENABLE_SCHEDULER.
...
def serve_main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(description="Coldview Server")
