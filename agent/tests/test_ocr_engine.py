...
must only ever be reachable through an explicit COLDVIEW_OCR_ENGINE
...
class TestEngineSelection:
    ...
    def test_none_disables_ocr(self, monkeypatch):
        monkeypatch.setenv("COLDVIEW_OCR_ENGINE", "none")
        reset_env_config()
    ...
    def test_auto_never_selects_cloud_engine(self, monkeypatch):
        ...
        monkeypatch.setenv("COLDVIEW_OCR_ENGINE", "auto")
    ...
    def test_unknown_choice_falls_back_to_local(self, monkeypatch):
        ...
        monkeypatch.setenv("COLDVIEW_OCR_ENGINE", "bogus-engine")
        reset_env_config()
    ...
    def test_llm_vision_available_with_api_key(self, monkeypatch):
        ...
        """
        monkeypatch.setenv("COLDVIEW_OCR_ENGINE", "llm-vision")
...
class TestProviderConfigResolution:
    ...
    def test_model_override_priority(self, monkeypatch):
        """COLDVIEW_OCR_LLM_MODEL overrides LANGCHAIN_MODEL_NAME."""
        monkeypatch.setenv("LANGCHAIN_PROVIDER", "openai")
        monkeypatch.setenv("LANGCHAIN_MODEL_NAME", "gpt-4o-mini")
        monkeypatch.setenv("OPENAI_API_KEY", "sk-test")
        monkeypatch.setenv("COLDVIEW_OCR_LLM_MODEL", "gpt-4o")
        reset_env_config()
...
class TestBackwardCompatAliases:
    ...
    def test_qwen_vl_alias_maps_to_llm_vision(self, monkeypatch, caplog):
        """COLDVIEW_OCR_ENGINE=qwen-vl should alias to llm-vision with a deprecation warning."""
        monkeypatch.setenv("COLDVIEW_OCR_ENGINE", "qwen-vl")
        monkeypatch.setenv("OPENAI_API_KEY", "sk-test")
        monkeypatch.setenv("LANGCHAIN_PROVIDER", "openai")
        monkeypatch.setenv("LANGCHAIN_MODEL_NAME", "gpt-4o")
        reset_env_config()
    ...
    def test_legacy_env_var_alias(self, monkeypatch, caplog):
        """COLDVIEW_OCR_QWEN_MODEL should alias to COLDVIEW_OCR_LLM_MODEL with deprecation warning."""
        monkeypatch.setenv("COLDVIEW_OCR_ENGINE", "llm-vision")
        monkeypatch.setenv("LANGCHAIN_PROVIDER", "openai")
        monkeypatch.setenv("LANGCHAIN_MODEL_NAME", "gpt-4o")
        monkeypatch.setenv("OPENAI_API_KEY", "sk-test")
        # Legacy env var
        monkeypatch.setenv("COLDVIEW_OCR_QWEN_MODEL", "qwen-vl-plus-legacy")
        # New env var NOT set
        monkeypatch.delenv("COLDVIEW_OCR_LLM_MODEL", raising=False)
        reset_env_config()
    ...
    def test_new_env_var_takes_precedence_over_legacy(self, monkeypatch):
        ...
        monkeypatch.setenv("LANGCHAIN_MODEL_NAME", "gpt-4o")
        monkeypatch.setenv("OPENAI_API_KEY", "sk-test")
        monkeypatch.setenv("COLDVIEW_OCR_QWEN_MODEL", "qwen-old")
        monkeypatch.setenv("COLDVIEW_OCR_LLM_MODEL", "gpt-4o-new")
        reset_env_config()
