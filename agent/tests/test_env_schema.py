...
class TestEnvConfigDefaults:
    ...
    def test_llm_defaults(self) -> None:
        ...
        assert c.llm.coldview_deepseek_adapter == "auto"
    ...
    def test_data_defaults(self) -> None:
        ...
        assert c.data.coldview_iwencai_key == ""
        assert c.data.coldview_sec_ua == ""
        assert c.data.coldview_data_cache is False
        assert c.data.coldview_data_cache_root == ""
    ...
    def test_api_defaults(self) -> None:
        ...
        assert c.api.coldview_api_key == ""
        assert c.api.cors_origins == ""
        assert c.api.api_allowed_hosts == ""
        assert c.api.enable_session_runtime is True
        assert c.api.coldview_trust_docker_loopback is False
        assert c.api.coldview_enable_shell_tools is False
        assert c.api.coldview_allowed_file_roots == ""
        assert c.api.coldview_allowed_write_roots == ""
        assert c.api.coldview_allowed_run_roots == ""
    ...
    def test_agent_tuning_defaults(self) -> None:
        ...
        assert c.agent_tuning.coldview_tool_timeout_seconds == 1800.0
        assert c.agent_tuning.coldview_goal_max_continuations == 3
        assert c.agent_tuning.coldview_sse_timeout == 90
        assert c.agent_tuning.content_filter_warning_threshold == 0.05
        assert c.agent_tuning.coldview_enable_advisory is False
        assert c.agent_tuning.coldview_enable_scheduler is False
        assert c.agent_tuning.coldview_channels_auto_start is False
        assert c.agent_tuning.coldview_disable_bottleneck is False
        assert c.agent_tuning.coldview_bench_workers == 0
        assert c.agent_tuning.coldview_search_backends == ""
        assert c.agent_tuning.coldview_search_bing_fallback is True

    def test_path_defaults(self) -> None:
        c = EnvConfig()
        assert c.paths.coldview_hypotheses_path == ""
        assert c.paths.coldview_goal_db_path == ""
        assert c.paths.coldview_swarm_agent_config == ""
...
class TestEnvConfigTypeCoercion:
    ...
    def test_bool_coercion_from_env(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("COLDVIEW_DATA_CACHE", "true")
        c = EnvConfig()
        assert c.data.coldview_data_cache is True
...
class TestAPIKeyAlias:
    """Verify COLDVIEW_API_KEY → api_auth_key alias resolution."""

    def test_coldview_api_key_only(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("COLDVIEW_API_KEY", "secret")
        c = EnvConfig()
        assert c.api.api_auth_key == "secret"
        assert c.api.coldview_api_key == "secret"
