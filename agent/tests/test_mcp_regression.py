...
2. MCP server mode regression — importing mcp_server.py must not raise, and
   ...
   confirms that the server-side MCP plugin (coldview-mcp) is unaffected
...
- TODO(v1): Add a live smoke test that spawns `coldview-mcp` as a stdio
...
# ---------------------------------------------------------------------------
# MCP server mode regression (coldview-mcp plugin)
# ---------------------------------------------------------------------------
...
def test_mcp_resolve_shell_tools_env_opt_in(monkeypatch: pytest.MonkeyPatch) -> None:
    """COLDVIEW_ENABLE_SHELL_TOOLS=1 turns shell tools on without the flag."""
    mod = _import_mcp_server()
...
def test_mcp_registry_omits_shell_tools_under_default_policy(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.delenv("COLDVIEW_ENABLE_SHELL_TOOLS", raising=False)
    mod = _import_mcp_server()
