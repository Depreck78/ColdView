...
def _clear_backend_env(monkeypatch):
    monkeypatch.delenv("COLDVIEW_SEARCH_BACKENDS", raising=False)
    monkeypatch.delenv("COLDVIEW_SEARCH_BING_FALLBACK", raising=False)
...
def test_env_overrides_backends(monkeypatch):
    """COLDVIEW_SEARCH_BACKENDS overrides the default engine list."""
    monkeypatch.setenv("COLDVIEW_SEARCH_BACKENDS", "google, bing")
    seen = {}
...
def test_persistent_failure_returns_actionable_error(monkeypatch):
    ...
    monkeypatch.setenv("COLDVIEW_SEARCH_BING_FALLBACK", "0")
    monkeypatch.setattr("src.tools.web_search_tool.time.sleep", lambda *_: None)
    calls = {"n": 0}
    ...
    assert "COLDVIEW_SEARCH_BACKENDS" in out["error"]
