...
def _client(tmp_path: Path, monkeypatch) -> TestClient:
    # Redirect the runtime root (``~/.coldview``) at the home boundary so the
    # live tree, HALT sentinel, mandate store, and proposal store all resolve
    # under tmp_path. get_runtime_root() == Path.home() / ".coldview".
...
def test_authorize_onramp_describes_cli_flow(tmp_path: Path, monkeypatch) -> None:
    ...
    assert "coldview connector authorize robinhood-live-mcp" in body["instruction"]
...
def _seed_proposal(tmp_path: Path, proposal_id: str, broker: str = "robinhood") -> dict:
    ...
    proposals_dir = tmp_path / ".coldview" / "live" / broker / "proposals"
...
def _seed_ledger(tmp_path: Path, record: dict) -> None:
    ledger = tmp_path / ".coldview" / "live" / "audit.jsonl"
