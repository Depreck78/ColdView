...
def test_backtest_runtime_env_keeps_market_data_configuration(
    ...
    allowed_values = {
        ...
        "COLDVIEW_IWENCAI_KEY": "iwencai-key",
        "COLDVIEW_SEC_UA": "Research Bot bot@example.com",
        "COLDVIEW_DATA_CACHE": "1",
        "COLDVIEW_ALLOWED_RUN_ROOTS": str(tmp_path),
        "COLDVIEW_FMP_MIN_INTERVAL": "0.5",
        "CCXT_EXCHANGE": "okx",
...
def test_backtest_runtime_env_scrubs_service_and_broker_secrets(
    ...
    sensitive_keys = [
        ...
        "API_AUTH_KEY",
        "COLDVIEW_API_KEY",
        "COLDVIEW_ENABLE_SHELL_TOOLS",
        "COLDVIEW_ENABLE_ADVISORY",
...
def test_sandbox_credentials_absent_in_this_environment() -> None:
    # No coldview-sandbox account here, so the UID-drop pre-check returns None and
...
def test_execute_uses_ephemeral_home_and_cleans_up(tmp_path: Path) -> None:
    ...
    assert "coldview-sandbox-home-" in home_line
...
def test_execute_falls_back_without_uid_drop_and_succeeds(tmp_path: Path) -> None:
    # With no coldview-sandbox user, execute() must NOT pass user=/group= and must
...
def test_execute_retries_without_uid_drop_when_drop_fails(
    ...
    # Simulate a host where coldview-sandbox exists but the drop is not permitted:
    # execute() must catch the failure, warn, and re-run without user=/group=.
    monkeypatch.setattr(
        runner_mod,
        "_resolve_sandbox_credentials",
        lambda: ("coldview-sandbox", "coldview-sandbox"),
    )
...
def test_execute_applies_address_space_rlimit(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setenv("COLDVIEW_SANDBOX_RLIMIT_AS_MB", "4096")
    run_dir = tmp_path / "run"
    run_dir.mkdir()
    entry = _probe_entry(
        tmp_path,
