...
def test_write_file_accepts_model_argument_aliases(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("COLDVIEW_ALLOWED_RUN_ROOTS", str(tmp_path))
...
def test_write_file_rejects_unconfigured_absolute_run_dir(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.delenv("COLDVIEW_ALLOWED_RUN_ROOTS", raising=False)
...
def test_tilde_expansion_resolves_to_mock_home(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("COLDVIEW_ALLOWED_WRITE_ROOTS", str(mock_home / ".coldview"))
    ...
    resolved = resolve_safe_path("~/.coldview/scripts/strat.py", None, allowed_write, purpose="write")
    assert resolved == mock_home / ".coldview" / "scripts" / "strat.py"
...
def test_read_write_separation_prevent_cross_escalation(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("COLDVIEW_ALLOWED_FILE_ROOTS", str(read_only_dir))
    monkeypatch.setenv("COLDVIEW_ALLOWED_WRITE_ROOTS", str(write_only_dir))
...
def test_resolve_safe_path_run_dir_escapes_fallback(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("COLDVIEW_ALLOWED_RUN_ROOTS", str(tmp_path / "runs"))
    monkeypatch.setenv("COLDVIEW_ALLOWED_WRITE_ROOTS", str(extra_write_dir))
