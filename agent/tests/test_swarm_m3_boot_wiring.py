...
The contract this file defends:
  ...
  * ``COLDVIEW_SWARM_AGENT_CONFIG`` env var, when set, wins absolutely —
    even if neighbouring ``swarm-agent.json`` / ``agent.json`` exist on disk.
  * Without the env var, ``~/.coldview/swarm-agent.json`` (the swarm-
    specific operator file) is preferred over ``agent.json``.
  * Without either swarm-specific file, ``~/.coldview/agent.json``
...
real ``~/.coldview`` directory of whoever is running the suite.
"""
...
def test_resolve_swarm_agent_config_path_uses_env_var_when_set(
    ...
    """``COLDVIEW_SWARM_AGENT_CONFIG`` is the absolute first-priority source.
    ...
    monkeypatch.setenv("COLDVIEW_SWARM_AGENT_CONFIG", str(env_pointed))
...
def test_resolve_swarm_agent_config_path_prefers_swarm_specific_file(
    ...
    """
    monkeypatch.delenv("COLDVIEW_SWARM_AGENT_CONFIG", raising=False)
    runtime_root = tmp_path / "runtime"
    _write_agent_json(runtime_root / "swarm-agent.json", "swarm_specific")
    _write_agent_json(runtime_root / "agent.json", "main_agent")
