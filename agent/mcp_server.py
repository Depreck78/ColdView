#!/usr/bin/env python3
"""Coldview MCP Server — expose finance research tools to any MCP client.
...
mcp = FastMCP("Coldview", version=APP_VERSION)
...
# COLDVIEW_ENABLE_SHELL_TOOLS env var. Keeping the module-level default off
...
def _env_shell_tools_enabled() -> bool:
    ...
    return get_env_config().api.coldview_enable_shell_tools
...
def _resolve_include_shell_tools(cli_opt_in: bool) -> bool:
    ...
    Returns:
        ...
        ``COLDVIEW_ENABLE_SHELL_TOOLS`` environment variable.
    """
...
def _parse_allowed_hosts(raw: str | None) -> list[str]:
    """Parse ``COLDVIEW_MCP_ALLOWED_HOSTS`` into a Host/Origin allow-list.
...
def add_goal_evidence(
    ...
    Args:
        ...
        run_id: Coldview run id. It verifies evidence only when the run directory exists.
...
# FRED_API_KEY, iwencai_search needs COLDVIEW_IWENCAI_KEY) is absent — see
...
# names the exact env var (``FRED_API_KEY`` / ``COLDVIEW_IWENCAI_KEY``).
...
def iwencai_search(query: str, limit: int = 20) -> str:
    ...
    Read-only; requires the COLDVIEW_IWENCAI_KEY access key (without it the
...
def main():
    """Entry point for `coldview-mcp` CLI command."""
    global _include_shell_tools, _registry
    import argparse

    parser = argparse.ArgumentParser(description="Coldview MCP Server")
    parser.add_argument(
        "--transport",
    ...
    parser.add_argument(
        ...
        "equivalent to setting COLDVIEW_ENABLE_SHELL_TOOLS=1.",
    )
    ...
    if args.transport in ("sse", "http"):
        ...
        allowed_hosts = _parse_allowed_hosts(
            get_env_config().api.coldview_mcp_allowed_hosts
        )
