...
UA defaults to a Coldview contact string overridable via
``COLDVIEW_SEC_UA``.
...
_MIN_INTERVAL_ENV = "COLDVIEW_SEC_MIN_INTERVAL"

_TICKERS_URL = "https://www.sec.gov/files/company_tickers.json"
_SUBMISSIONS_URL = "https://data.sec.gov/submissions/CIK{cik}.json"
_COMPANY_FACTS_URL = "https://data.sec.gov/api/xbrl/companyfacts/CIK{cik}.json"

_UA_ENV = "COLDVIEW_SEC_UA"
...
_DEFAULT_SEC_UA = f"Coldview/1.0 (contact: coldview@example.com) {DEFAULT_USER_AGENT}"
...
def _user_agent() -> str:
    """Return the compliant contact UA, honoring the ``COLDVIEW_SEC_UA`` override."""
    from src.config.accessor import get_env_config

    override = get_env_config().data.coldview_sec_ua
