...
> **Package name vs commands:** The PyPI package is `coldview-ai`. Once installed, you get:
>
> | Command | Purpose |
> |---------|---------|
...
Each alpha ships with `__alpha_meta__` (formula LaTeX + theme + universe + warmup + columns required), guarded by an AST purity gate + 300-row lookahead sentinel test. Use the `coldview alpha {list,show,bench,compare,export-manifest}` CLI, the `/alpha/*` REST routes (browser at `/alpha-zoo`), or compose multi-factor signals via `ZooSignalEngine.from_zoo(...)`.
...
That's it — no API keys needed for HK/US/crypto markets. Start using `backtest`, `get_market_data`, `analyze_options`, `analyze_trade_journal`, `extract_shadow_strategy`, `web_search`, the **Alpha Zoo** (`coldview alpha bench --zoo gtja191 --universe csi300 --period 2018-2025`), and all 89 skills immediately.
...
> **Note:** This is the *MCP client* path — the opposite of the MCP plugin listed above. The plugin above makes Coldview's tools available to your agents. This section lets Coldview's own agent call tools from *your* servers.
...
Ordinary external MCP tools appear automatically in every `coldview run` / `coldview chat` call. They are injected after local tools under stable names: `mcp_<server>_<tool>`. Live-broker MCP servers are consumed through the connector-scoped `trading_*` tools instead of exposing raw `mcp_<broker>_*` tools to the agent.
...
Authorize it with `coldview connector authorize ibkr-live-official-mcp-readonly`. The wildcard is accepted
only for this `mcp.read` probe. Generic `trading_account` and `trading_positions`
calls stay disabled until IBKR publishes stable read tool names that Coldview
...
Without `ALLOW_SESSION_MCP_SERVERS=1`, any `mcpServers` key in `session.config` is silently stripped before config loading. The global operator config on disk (`~/.coldview/agent.json`) is always respected regardless of this flag.
