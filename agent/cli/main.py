"""Interactive CLI front door for Coldview.
...
The console script entry in ``pyproject.toml`` (``coldview = "cli:main"``)
hits :func:`main`.
"""
...
_ENV_PATH = Path.home() / ".coldview" / ".env"
...
def _probe_session_count() -> int:
    ...
    db_path = Path.home() / ".coldview" / "sessions.db"
...
def _show_banner() -> None:
    ...
    console.print(
        ...
        "[/dim][bold]coldview connector --help[/bold][dim] · "
...
def _new_session(prompt_preview: str) -> Optional[str]:
    ...
    (``~/.coldview/sessions.db``) so cross-session search via
...
def _start_preflight_async() -> threading.Thread:
    ...
    thread = threading.Thread(target=_worker, daemon=True, name="coldview-preflight")
    thread.start()
    return thread
...
def _run_one_turn(user_input: str, ctx: InteractiveContext) -> None:
    ...
    try:
        ...
        dashboard.close()
        # BrokenPipe: caller did ``coldview chat | head`` and the
...
def _trip_halt_from_repl(console: Any, *, reason: str) -> None:
    ...
    console.print(
        ...
        "disabled until you run [bold]/resume[/bold] or [bold]coldview connector resume[/bold]."
    )
...
def _clear_halt_from_repl(console: Any) -> None:
    ...
    path so the model never performs it. Mirrors ``coldview connector resume``
...
def _run_connector_command_from_repl(console: Any, args: list[str]) -> None:
    ...
    ``/connector`` is a thin in-REPL bridge to the ``coldview connector``
...
def _commit_mandate(proposal: Dict[str, Any], selected_ordinal: int) -> Dict[str, Any]:
    ...
    The endpoint base URL is read from ``COLDVIEW_API_URL`` (falling back to
    ...
    base = api_config.coldview_api_url.rstrip("/")
...
def _interactive_loop(max_iter: int, resume_session_id: Optional[str] = None) -> int:
    ...
    if resume_session_id:
        # Resume a specific session by ID (``coldview resume <session-id>``).
        try:
            store = _session_store()
    ...
    if ctx.session_id:
        console.print(
            f"[dim]To resume this session:[/dim] [bold]coldview resume {ctx.session_id}[/bold]"
        )
    return 0
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    * Non-interactive entry (``serve``, ``run -p ...``, ``mcp``,
    ...
    # Handle ``coldview resume <session-id>`` — enter the interactive
...
def _build_typer_app():  # type: ignore[no-untyped-def]
    ...
    app = typer.Typer(
        ...
        help="Coldview — natural-language finance research agent.",
